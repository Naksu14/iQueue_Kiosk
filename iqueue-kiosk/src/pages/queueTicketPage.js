...
const QueueTicketPage = () => {
  ...
  const [countWaiting, setCountWaiting] = useState(null);
  const [avgServiceTime, setAvgServiceTime] = useState(null);

  useEffect(() => {
    const fetchCount = async () => {
      if (!transactions || transactions.length === 0) return;
      // Prefer officeId (requested). Fall back to officeName only if id not available.
      const officeId = transactions[0].officeId || transactions[0].office_id;
      const officeName =
        transactions[0].officeName || transactions[0].office_name;

      if (!officeId && !officeName) return;
      try {
        // Try to call APIs with officeId when available (preferred)
        const countRes = await getCountWaiting(officeName);
        const avgRes = await getAverageServiceTime(officeId);
        const normalized =
          typeof countRes === "number" ? countRes : countRes?.count ?? countRes;
        setCountWaiting(normalized);
        const normalizedAvg =
          typeof avgRes === "number"
            ? avgRes
            : avgRes?.average ?? avgRes?.avg ?? avgRes;
        setAvgServiceTime(normalizedAvg);
        // If you want to show estimated wait later, you can compute it using avgRes
      } catch (err) {
        console.warn("Failed to fetch waiting count:", err);
      }
    };
  ...
  const formatEstimatedTime = (sec) => {
    ...
    const remMin = min % 60;
    return `${hr} hr${hr > 1 ? "s" : ""}${remMin > 0 ? ` ${remMin} min` : ""}`;
  };

  const estimatedSeconds =
    avgServiceTime && countWaiting
      ? Number(avgServiceTime) * Number(countWaiting)
      : null;
    ...
    <div className="flex flex-col items-center justify-center ">
      ...
      <div className="flex gap-4 w-full justify-center items-center">
        <div className="flex flex-col items-center justify-center rounded-lg py-2 w-64 ">
          {printStatus === "idle" && (
            <>
              <div className="bg-white/90 backdrop-blur-md shadow-xl border border-gray-200 rounded-2xl p-4 w-[320px] text-center transition-all hover:shadow-2xl hover:scale-[1.02] duration-300">
                ...
                <div className="mt-3 text-center w-full">
                  {countWaiting === null ? null : (
                    <div className="text-sm text-gray-600">
                      {countWaiting > 0 ? (
                        <div className="flex flex-col justify-center items-center gap-1">
                          <span className="font-medium text-gray-800">
                            {countWaiting} in queue
                          </span>
                          {estimatedSeconds ? (
                            ...
                            <span className="text-sm text-orange-500 font-semibold">
                              Avg service: {formatAvgMinutes(avgServiceTime)}
                            </span>
                          ) : null}
                        </div>
                      ) : (
              ...
              <button
                onClick={handlePrint}
                className="w-full h-14 flex items-center justify-center mt-2 rounded-full
          ...
          {printStatus === "no-paper" && (
            <>
              ...
              <p className="text-gray-600 text-sm text-center mb-4 w-[260px]">
                We apologize! The ticket printer is currently out of paper.
